package via.winmall.lueorganisation.adapter;

import java.util.List;

public class ItemEntity {
    private String id;
    private String parent_id;
    private String name;
    private String is_active;
    private String position;
    private String level;
    private String product_count;
    private String children_data;
    private String category_name;
    private String status;
    private String SalePrice;
    private String MrpPrice;
    private int Image1;
    private String Image;
    private String Ratingcount;
    private String Description;
    private String ItemColor;
    private String ItemHeight;
    private String ItemMateial;
    private String ItemStock;
    private String UserLoginId;
    private String UserLoginPass;
    private String UserFirstName;
    private String UserId;
    private String sliderImage;
    private String productPromotionName;
    private String productPromotionImage;
    private String productPromotionId;
    private String verifyId;
    private String Product_Url;
    private String Category_id;
    private String Increment_id;
    private String Order_status;
    private String Coupon_code;
    private String Total_item_count;
    private String Total_qty_ordered;
    private String Shipping_amount;
    private String Subtotal;
    private String Grand_total;
    private String Shipping_method;
    private String Ordered_qty;
    private String Price;
    private String Order_created_date;
    private String ShippingAddress;
    private String FirstName;
    private String LastName;
    private String City;
    private String Street;
    private String Country;
    private String Telephone;
    private String Email;
    private String BrandName;
    private String BrandId;
    private String langitude;
    private String longitutde;
    private List<String> ImageListString;
    boolean selected = false;
    private List<images> ImageList;
    private String TitleReview;
    private String ReviewDetail;
    private String Rating;
    private String CustomerName;
    private String Newproductname;


    String nameproduct;
    String color;
    String model;
    String brand;
    String efficacyPurposes;
    String incenseMaterial;
    String ScentFlavour;
    String packaging;
    String dimensionVolume;
    String region;
    String patentRegistered;
    String imported;
    String certification;
    String netweight;
    String price;


    public boolean isSelected() {
        return selected;
    }

    public void setSelected(boolean selected) {
        this.selected = selected;
    }

    public String getNameproduct() {
        return nameproduct;
    }

    public void setNameproduct(String nameproduct) {
        this.nameproduct = nameproduct;
    }

    public String getColor() {
        return color;
    }

    public void setColor(String color) {
        this.color = color;
    }

    public String getModel() {
        return model;
    }

    public void setModel(String model) {
        this.model = model;
    }

    public String getBrand() {
        return brand;
    }

    public void setBrand(String brand) {
        this.brand = brand;
    }

    public String getEfficacyPurposes() {
        return efficacyPurposes;
    }

    public void setEfficacyPurposes(String efficacyPurposes) {
        this.efficacyPurposes = efficacyPurposes;
    }

    public String getIncenseMaterial() {
        return incenseMaterial;
    }

    public void setIncenseMaterial(String incenseMaterial) {
        this.incenseMaterial = incenseMaterial;
    }

    public String getScentFlavour() {
        return ScentFlavour;
    }

    public void setScentFlavour(String scentFlavour) {
        ScentFlavour = scentFlavour;
    }

    public String getPackaging() {
        return packaging;
    }

    public void setPackaging(String packaging) {
        this.packaging = packaging;
    }

    public String getDimensionVolume() {
        return dimensionVolume;
    }

    public void setDimensionVolume(String dimensionVolume) {
        this.dimensionVolume = dimensionVolume;
    }

    public String getRegion() {
        return region;
    }

    public void setRegion(String region) {
        this.region = region;
    }

    public String getPatentRegistered() {
        return patentRegistered;
    }

    public void setPatentRegistered(String patentRegistered) {
        this.patentRegistered = patentRegistered;
    }

    public String getImported() {
        return imported;
    }

    public void setImported(String imported) {
        this.imported = imported;
    }

    public String getCertification() {
        return certification;
    }

    public void setCertification(String certification) {
        this.certification = certification;
    }

    public String getNetweight() {
        return netweight;
    }

    public void setNetweight(String netweight) {
        this.netweight = netweight;
    }

    public String getNewproductname() {
        return Newproductname;
    }

    public void setNewproductname(String newproductname) {
        Newproductname = newproductname;
    }

    public String getNewproductimage() {
        return Newproductimage;
    }

    public void setNewproductimage(String newproductimage) {
        Newproductimage = newproductimage;
    }

    public String getNewproductid() {
        return Newproductid;
    }

    public void setNewproductid(String newproductid) {
        Newproductid = newproductid;
    }

    private String Newproductimage;

    private String Newproductid;


    public List<images> getImageList() {
        return ImageList;
    }

    public void setImageList(List<images> imageList) {
        ImageList = imageList;
    }

    public String getDescription() {
        return Description;
    }

    public void setDescription(String description) {
        Description = description;
    }

   

    public String getItemHeight() {
        return ItemHeight;
    }

    public void setItemHeight(String itemHeight) {
        ItemHeight = itemHeight;
    }

    public String getItemColor() {
        return ItemColor;
    }

    public void setItemColor(String itemColor) {
        ItemColor = itemColor;
    }

    public String getItemMateial() {
        return ItemMateial;
    }

    public void setItemMateial(String itemMateial) {
        ItemMateial = itemMateial;
    }

    public String getItemStock() {
        return ItemStock;
    }

    public void setItemStock(String itemStock) {
        ItemStock = itemStock;
    }

    public String getUserLoginId() {
        return UserLoginId;
    }

    public void setUserLoginId(String userLoginId) {
        UserLoginId = userLoginId;
    }

    public String getUserLoginPass() {
        return UserLoginPass;
    }

    public void setUserLoginPass(String userLoginPass) {
        UserLoginPass = userLoginPass;
    }

    public String getUserFirstName() {
        return UserFirstName;
    }

    public void setUserFirstName(String userFirstName) {
        UserFirstName = userFirstName;
    }

    public String getUserId() {
        return UserId;
    }

    public void setUserId(String userId) {
        UserId = userId;
    }

    public String getSliderImage() {
        return sliderImage;
    }

    public void setSliderImage(String sliderImage) {
        this.sliderImage = sliderImage;
    }

    public String getProductPromotionImage() {
        return productPromotionImage;
    }

    public String getProductPromotionName() {
        return productPromotionName;
    }

    public void setProductPromotionName(String productPromotionName) {
        this.productPromotionName = productPromotionName;
    }

    public void setProductPromotionImage(String productPromotionImage) {
        this.productPromotionImage = productPromotionImage;
    }

    public String getProductPromotionId() {
        return productPromotionId;
    }

    public void setProductPromotionId(String productPromotionId) {
        this.productPromotionId = productPromotionId;
    }

    public String getVerifyId() {
        return verifyId;
    }

    public void setVerifyId(String verifyId) {
        this.verifyId = verifyId;
    }

    public String getProduct_Url() {
        return Product_Url;
    }

    public void setProduct_Url(String product_Url) {
        Product_Url = product_Url;
    }

    public String getCategory_id() {
        return Category_id;
    }

    public void setCategory_id(String category_id) {
        Category_id = category_id;
    }

    public String getIncrement_id() {
        return Increment_id;
    }

    public void setIncrement_id(String increment_id) {
        Increment_id = increment_id;
    }

    public String getOrder_status() {
        return Order_status;
    }

    public void setOrder_status(String order_status) {
        Order_status = order_status;
    }

    public String getCoupon_code() {
        return Coupon_code;
    }

    public void setCoupon_code(String coupon_code) {
        Coupon_code = coupon_code;
    }

    public String getTotal_item_count() {
        return Total_item_count;
    }

    public void setTotal_item_count(String total_item_count) {
        Total_item_count = total_item_count;
    }

    public String getTotal_qty_ordered() {
        return Total_qty_ordered;
    }

    public void setTotal_qty_ordered(String total_qty_ordered) {
        Total_qty_ordered = total_qty_ordered;
    }

    public String getShipping_amount() {
        return Shipping_amount;
    }

    public void setShipping_amount(String shipping_amount) {
        Shipping_amount = shipping_amount;
    }

    public String getSubtotal() {
        return Subtotal;
    }

    public void setSubtotal(String subtotal) {
        Subtotal = subtotal;
    }

    public String getGrand_total() {
        return Grand_total;
    }

    public void setGrand_total(String grand_total) {
        Grand_total = grand_total;
    }

    public String getShipping_method() {
        return Shipping_method;
    }

    public void setShipping_method(String shipping_method) {
        Shipping_method = shipping_method;
    }

    public String getOrdered_qty() {
        return Ordered_qty;
    }

    public void setOrdered_qty(String ordered_qty) {
        Ordered_qty = ordered_qty;
    }

    public String getPrice() {
        return Price;
    }

    public void setPrice(String price) {
        Price = price;
    }

    public String getOrder_created_date() {
        return Order_created_date;
    }

    public void setOrder_created_date(String order_created_date) {
        Order_created_date = order_created_date;
    }

    public String getShippingAddress() {
        return ShippingAddress;
    }

    public void setShippingAddress(String shippingAddress) {
        ShippingAddress = shippingAddress;
    }

    public String getFirstName() {
        return FirstName;
    }

    public void setFirstName(String firstName) {
        FirstName = firstName;
    }

    public String getLastName() {
        return LastName;
    }

    public void setLastName(String lastName) {
        LastName = lastName;
    }

    public String getCity() {
        return City;
    }

    public void setCity(String city) {
        City = city;
    }

    public String getStreet() {
        return Street;
    }

    public void setStreet(String street) {
        Street = street;
    }

    public String getCountry() {
        return Country;
    }

    public void setCountry(String country) {
        Country = country;
    }

    public String getTelephone() {
        return Telephone;
    }

    public void setTelephone(String telephone) {
        Telephone = telephone;
    }

    public String getEmail() {
        return Email;
    }

    public void setEmail(String email) {
        Email = email;
    }

    public String getBrandName() {
        return BrandName;
    }

    public void setBrandName(String brandName) {
        BrandName = brandName;
    }

    public String getBrandId() {
        return BrandId;
    }

    public void setBrandId(String brandId) {
        BrandId = brandId;
    }

    public List<String> getImageListString() {
        return ImageListString;
    }

    public void setImageListString(List<String> imageListString) {
        ImageListString = imageListString;
    }

    public String getLongitutde() {
        return longitutde;
    }

    public void setLongitutde(String longitutde) {
        this.longitutde = longitutde;
    }

    public String getLangitude() {
        return langitude;
    }

    public void setLangitude(String langitude) {
        this.langitude = langitude;
    }

    public String getTitleReview() {
        return TitleReview;
    }

    public void setTitleReview(String titleReview) {
        TitleReview = titleReview;
    }

    public String getReviewDetail() {
        return ReviewDetail;
    }

    public void setReviewDetail(String reviewDetail) {
        ReviewDetail = reviewDetail;
    }

    public String getCustomerName() {
        return CustomerName;
    }

    public void setCustomerName(String customerName) {
        CustomerName = customerName;
    }

    public String getRating() {
        return Rating;
    }

    public void setRating(String rating) {
        Rating = rating;
    }


    public static class images{
        private String name;
        private String src;

        public String getName() {
            return name;
        }

        public void setName(String name) {
            this.name = name;
        }

        public String getSrc() {
            return src;
        }

        public void setSrc(String src) {
            this.src = src;
        }
    }

    public String getId() {
        return id;
    }

    public void setId(String id) {
        this.id = id;
    }

    public String getParent_id() {
        return parent_id;
    }

    public void setParent_id(String parent_id) {
        this.parent_id = parent_id;
    }

    public String getName() {
        return name;
    }

    public void setName(String name) {
        this.name = name;
    }

    public String getIs_active() {
        return is_active;
    }

    public void setIs_active(String is_active) {
        this.is_active = is_active;
    }

    public String getPosition() {
        return position;
    }

    public void setPosition(String position) {
        this.position = position;
    }

    public String getLevel() {
        return level;
    }

    public void setLevel(String level) {
        this.level = level;
    }

    public String getProduct_count() {
        return product_count;
    }

    public void setProduct_count(String product_count) {
        this.product_count = product_count;
    }

    public String getChildren_data() {
        return children_data;
    }

    public void setChildren_data(String children_data) {
        this.children_data = children_data;
    }

    public String getStatus() {
        return status;
    }

    public void setStatus(String status) {
        this.status = status;
    }

    public String getCategory_name() {
        return category_name;
    }

    public void setCategory_name(String category_name) {
        this.category_name = category_name;
    }

    public String getMrpPrice() {
        return MrpPrice;
    }

    public void setMrpPrice(String mrpPrice) {
        MrpPrice = mrpPrice;
    }

    public String getSalePrice() {
        return SalePrice;
    }

    public void setSalePrice(String salePrice) {
        SalePrice = salePrice;
    }

    public int getImage1() {
        return Image1;
    }

    public void setImage1(int image1) {
        Image1 = image1;
    }
    public String getImage() {
        return Image;
    }

    public void setImage(String image) {
        Image = image;
    }

    public String getRatingcount() {
        return Ratingcount;
    }

    public void setRatingcount(String ratingcount) {
        Ratingcount = ratingcount;
    }
}
